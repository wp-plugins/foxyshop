...
function foxyshop_google_products_page() {
	...
	<?php
		...
		<div class="wrap">
		<div class="icon32" id="icon-tools"><br></div>
		<h2>Google Products Management <a class="<?php if (version_compare(get_bloginfo('version'), '3.1', "<=")) echo "button "; ?>add-new-h2" href="edit.php?post_type=foxyshop_product&amp;page=foxyshop_google_products_page&amp;foxyshop_google_products_update=1&amp;googleprodlogout=1&amp;_wpnonce=<?php echo wp_create_nonce('gp1'); ?>">De-Authenticate</a></h2>
		<?php
	...
	<?php if (count($local_products) > 0) { ?>
		...
		$args = array('post_type' => 'foxyshop_product', 'post_status' => 'publish', 'numberposts' => "-1", "orderby" => "id", "order" => "ASC", "post__in" => $local_products);
		$product_list = get_posts($args);
		foreach ($product_list as $single_product) {
